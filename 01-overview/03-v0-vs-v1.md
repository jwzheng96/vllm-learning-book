...
> **谁该读这一篇？** 打开 vLLM 源码看到 `vllm/engine/` 和 `vllm/v1/` 两个目录犹豫"该读哪个"的读者；想系统理解 V0 和 V1 差异的工程师。
>
...
> **当前源码复核（`b23bd73`）：** 新的生成式推理主线以 V1 为准，但“路径不含 `/v1/` 就是 V0”已经不是可靠判据：配置、入口、模型层、分布式和通用工具仍是共享模块。迁移判断应以调用关系和类契约为准，而不是只看目录名。
...
**学习引擎与调度时优先读 V1。** 但入口、配置、模型实现、分布式和算子仍大量位于 `vllm/` 的共享目录；不要把“V1 主路径”误读为“只需要读 `vllm/v1/`”。本笔记未特别说明时，Engine 与 Scheduler 指 V1 实现。
...
3. 解释 "V0/V1 区别" 时，按上面 5 点框架展开，最后落一句话："本质是把*调度策略*与*执行机制*彻底解耦"。
...
→ **判断"V0 vs V1"的可靠方法**：从公开入口沿调用链确认对象最终落到 `vllm.v1.engine` / `vllm.v1.core.sched`，同时承认配置、模型与分布式代码是共享层；目录名只能当线索。
