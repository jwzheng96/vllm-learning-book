# 13. 384 卡 H100 实战：GLM-5.1 / GLM-5.2 / DeepSeek-V4-Flash 分布式生产部署
...
> 2. 为三个模型算清权重、KV cache、运行时余量，并选择 `PP×TP` 或 `DP×EP`
...
### 1.3 为什么不把 384 卡全放进一个 communicator
...
$$
M_{HBM}=M_{weight}+M_{KV}+M_{activation}+M_{graph}+M_{comm}+M_{fragment}
$$

权重粗估只能用于第一轮排除：

$$
M_{weight}\approx N_{param}\times bytes\_per\_param+scale+metadata
$$
...
1. GLM BF16 约 1.5 TB，16×H100 的物理 HBM 只有 1.28 TB，**三模型教程的 H100 基线必须用官方 FP8 checkpoint**。若必须 BF16，至少从 3 节点 `PP3×TP8` 重新做容量与性能验证。
...
$$
GPUSeconds/request = GPUs_{replica}\times E[T_{service}]
$$

$$
GPU_{need}=\frac{\lambda\times E[GPUSeconds/request]}{U_{target}}\times H_{burst}
$$
...
不要把三个模型塞进同一个“万能 latest 镜像”。独立镜像会多占一些节点缓存，却能让依赖、回滚和 blast radius 可解释。CUDA 12.9/13.0 的选择必须匹配已验证 driver；镜像能被 driver 启动不等于所有自定义 kernel 都兼容。
...
$$
N_P:N_D \approx \lambda E[T_P] : \lambda E[T_D]
$$
...
退出条件：三个模型都有可复现启动脚本、golden 结果、饱和点和已知限制。
...
TP 是为单副本放不下服务；三个模型分别能在 16/16/8 卡单元放下。TP384 会把逐层 collective 和故障域扩大到全池，吞吐应靠独立副本扩展。
...
- [ ] 三个模型都固定 revision，所有 shard 校验通过
...
- 384×H100 应组织成 48 个 8 卡节点和多个小故障域：GLM-5.1/5.2 以两节点 `PP2×TP8` 为起点，DeepSeek-V4-Flash 以单节点 `DP8+EP` 为起点，吞吐通过 Gateway 外部 DP 扩展。
...
1. 画出 384 卡三模型混部图，并说明为什么 GLM 的 TP 不跨节点、DeepSeek 的 EP 不跨 48 节点。
