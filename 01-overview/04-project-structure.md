...
> **谁该读这一篇？** 第一次 clone 完 vLLM 仓库、被持续增长的 Python 与 CUDA/C++ 文件吓退的同学；后续每一篇源码走读前，需要先知道"我要找的东西在哪一级目录"的读者。
>
...
> **当前源码复核（`b23bd73`）：** 目录职责和关键入口已按锁定树核对。文件/模型数量属于瞬时统计，本章不再把它们当架构事实；需要数量时请在同一 commit 上运行 `find vllm -name '*.py'`、`find csrc -type f` 等可复现命令。

vLLM 仓库规模很大且会随主线快速变化。第一次打开仓库容易迷路。本节给你一张"地图"——每个目录、每个关键文件干什么。找东西时回头查这一篇就行。
...
### 4.5 `model_executor/models/`：模型实现（数量以锁定树扫描为准）
